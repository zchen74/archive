! $Id: tagged_ox_adj_mod.f,v 1.1 2010/03/09 15:03:47 daven Exp $
      MODULE TAGGED_OX_ADJ_MOD
!
!******************************************************************************
!  Module TAGGED_OX_ADJ_MOD contains variables and routines for the adjoint of
!  the Ox simulation (lzh, dkh, 03/08/10)
!  simulation.
!
!  Based on forward model. (amf,rch,bmy, 8/20/03, 12/4/07)
!
!  Module Variables:
!  ============================================================================
...
!
      ...
      PRIVATE :: INIT_TAGGED_OX_ADJ
      PRIVATE :: READ_POX_LOX_ADJ
...
#     include "CMN_DIAG" ! LD65
 ...
 100  FORMAT( '     - READ_POX_LOX_ADJ: Reading ', a )

      ! Get the TAU0 value for today
      ! lzh 04/03/2008  - now backwards
      XTAU = GET_TAU()- ( 24-GET_TS_CHEM()/60d0 )
                                !23 for 60 min; 23.5 for 30 min
      print*, 'TAU0: ', XTAU
      print*, ' WARNING:  NHMSe must be 000000 for the XTAU '
      print*, '           to be correct '
     ...
     &                 LLTROP,    ARRAY,     QUIET=.TRUE. )
      ...
      ! Return to calling program
      END SUBROUTINE READ_POX_LOX_ADJ
...
!                              350 hPa for GEOS-5, GEOS-FP and MERRA2
!******************************************************************************
!
      ! References to F90 modules
...
!
      ...
      USE TRACER_MOD,   ONLY : N_TRACERS, XNUMOL
      USE TRACERID_MOD, ONLY : IDTOX
      USE TROPOPAUSE_MOD, ONLY : ITS_IN_THE_TROP

      ! lzh, 12/12/2009, add tagged ox adjoint
      USE ADJ_ARRAYS_MOD, ONLY : EMS_SF_ADJ
      USE ADJ_ARRAYS_MOD, ONLY : STT_ADJ
      USE ADJ_ARRAYS_MOD, ONLY : IDADJ_POx
...
!                     ENDIF
            ...
            !===========================================================
            !lzh, 12/12/2009, adjoint chemistry for tagged ox
            STT_ADJ(I,J,L,N)    = STT_ADJ(I,J,L,N) - LL
            EMS_SF_ADJ(I,J,L,N) = EMS_SF_ADJ(I,J,L,IDADJ_POx) +
     &                            STT_ADJ(I,J,L,N) * PP(I,J,L,N)

               ! dkh debug
               !IF ( I == IFD .and. J == JFD .and. L == LFD ) THEN
               !   print*, ' PP SUM adj = ', SUM(PP(I,J,L,:)), N
               !ENDIF
...
!         ENDIF
      ...
      ! Return to calling program
      END SUBROUTINE CHEM_TAGGED_OX_ADJ

!------------------------------------------------------------------------------

      SUBROUTINE INIT_TAGGED_OX_ADJ
!
!******************************************************************************
...
#     include "CMN_DIAG"  ! ND44, ND65, LD65
     ...
     &                    'INIT_TAGGED_OX (tagged_ox_mod.f)' )
      ...
      ! Return to calling program
      END SUBROUTINE INIT_TAGGED_OX_ADJ

!------------------------------------------------------------------------------

      SUBROUTINE CLEANUP_TAGGED_OX_ADJ
!
!******************************************************************************
...
!
      ...
      ! Return to calling program
      END SUBROUTINE CLEANUP_TAGGED_OX_ADJ

!------------------------------------------------------------------------------

      END MODULE TAGGED_OX_ADJ_MOD
