...
#include "CMN_SIZE"      ! Size parameters
  ...
  SUBROUTINE READ_STANDARD_DEVIATIONS
    ...
    RESULT = NF90_OPEN( "wc_std/CO_monthly_STDEV_del.20060331.nc", NF90_NOWRITE, FILE_ID )
    ...
    !! only use standard deviation values between model level 18 and 31
    ...
    DO L=1,LLPAR
       DO J=1,JJPAR
          DO I=1,IIPAR
             ...
             IF( L<18 ) THEN
                WC_SIGMA(I,J,L) = WC_STD_DEV(I,J,18,1)
             ELSE IF ( L>31 ) THEN
       ...
       DO J = MIN_LAT_U_INDEX, MAX_LAT_U_INDEX
          DO I = MIN_LON_U_INDEX, MAX_LON_U_INDEX
             ...
             IF ( L< ( MIN_LEV_U_INDEX + 3 ) ) THEN
                ...
                X_U_TEMP( I_DUM , CT_MAIN_U) = FORCE_U_FULLGRID( I, J, L )/( (WC_SIGMA(I,J,L)*1/(1+MIN_LEV_U_INDEX + 3-L) )**2)
                COST_FUNC = COST_FUNC +  FORCE_U_FULLGRID( I, J, L)**2 /( (WC_SIGMA(I,J,L) *1/(1+MAX_LEV_U_INDEX + 3 - L) )**2)
                
             ELSE IF ( L > ( MAX_LEV_U_INDEX - 3 ) ) THEN
                
                X_U_TEMP( I_DUM , CT_MAIN_U) = FORCE_U_FULLGRID( I, J, L ) /( (WC_SIGMA(I,J,L)*1/(1+MAX_LEV_U_INDEX - 3 + L) )**2)         
                COST_FUNC = COST_FUNC +  FORCE_U_FULLGRID( I, J, L )**2 /( (WC_SIGMA(I,J,L) *1/(1+MAX_LEV_U_INDEX - 3 + L) )**2)
                
             ELSE
                
                X_U_TEMP( I_DUM , CT_MAIN_U) = FORCE_U_FULLGRID( I, J, L ) /( WC_SIGMA(I,J,L) **2)         
                COST_FUNC = COST_FUNC +  FORCE_U_FULLGRID( I, J, L )**2 /( WC_SIGMA(I,J,L) **2)
