...
!      REAL*8,            ALLOCATABLE :: STT(:,:,:,:)
      REAL*8,            ALLOCATABLE, TARGET :: STT(:,:,:,:)   ! (lzh,11/01/2014) in v9-02
      ...
      REAL*8,            ALLOCATABLE :: TMP_PRESS(:,:)
      REAL*8,            ALLOCATABLE :: FP(:,:)
      INTEGER,           ALLOCATABLE :: IM(:,:)

      ! Strat prod and loss (hml)
      REAL*8,            ALLOCATABLE :: STT_STRAT_TMP(:,:,:,:)
      !-----------------------------------------------|
...
!$OMP CRITICAL
            LNEG = .FALSE.  !zc
         !   WRITE( 6, 100 ) I, J, L, N, STT(I,J,L,N)!zc
         !   PRINT*, STT(I,J,L,N) !zc
!$OMP END CRITICAL
...
!******************************************************************************
!
!  Subroutine CHECK\_STT\_05x0666 checks the STT tracer array for
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE CHECK_STT_025x03125( LOCATION )
!
! !USES:
!
      USE ERROR_MOD,          ONLY : IT_IS_NAN
...
!------------------------------------------------------------------------------
!BOC
!!! (lzh, 11/01/2014) add for 0.25 nested
!
! !LOCAL VARIABLES:
!
      INTEGER         :: I, J, L, N
