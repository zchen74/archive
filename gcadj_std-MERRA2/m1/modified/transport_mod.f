...
!
      ...
      PUBLIC :: INIT_MERRA2_WINDOW_TRANSPORT  ! (mps,11/13/2015)
...
#     include "CMN_SIZE" ! Size parameters
      ...
      CALL INIT_MERRA2_WINDOW_TRANSPORT  !(mps,11/13/2015)
...
#elif defined( GRID05x0625 )
         CALL DO_MERRA2_WINDOW_TRANSPORT    !(mps,11/13/2015)
#elif defined( GRID025x03125 )
...
!------------------------------------------------------------------------------
! add merra2 nested simulation (mps, 11/13/2015)

      SUBROUTINE DO_MERRA2_WINDOW_TRANSPORT
!
!******************************************************************************
!  Subroutine GEOS4_GEOS_5_GLOBAL_ADV is the driver routine for TPCORE with
!  the GMAO GEOS-4 or GEOS-5 met fields. (bmy, 10/30/07)
...
!
      ...
      USE LOGICAL_MOD,               ONLY : LEMBED, LFILL, LMFCT
      USE LOGICAL_MOD,               ONLY : LPRT,   LWINDO
      USE PJC_PFIX_MERRA2_WINDOW_MOD, ONLY : DO_PJC_PFIX_MERRA2_WINDOW  !!!
      USE PRESSURE_MOD,              ONLY : GET_PEDGE
      USE PRESSURE_MOD,              ONLY : SET_FLOATING_PRESSURE
      USE TIME_MOD,                  ONLY : GET_TS_DYN
      USE TPCORE_BC_MOD,             ONLY : I0_W, J0_W, I1_W, J1_W
      ...
      USE TPCORE_BC_MOD,             ONLY : DO_WINDOW_TPCORE_BC
      USE TPCORE_WINDOW_MOD,         ONLY : TPCORE_WINDOW
      USE TPCORE_MERRA2_WINDOW_MOD,  ONLY : TPCORE_MERRA2_WINDOW
      USE TPCORE_MERRA2_WINDOW_MOD,  ONLY : TPCORE_MERRA2_WINDOW
      USE TRACER_MOD,                ONLY : N_TRACERS, STT, TCVV
...
!$OMP END PARALLEL DO
      ...
      ! NOTE: P_TP1 and P_TP2 are the true surface pressures!
      CALL DO_PJC_PFIX_MERRA2_WINDOW( D_DYN, P_TP1, P_TP2,
     &                               UWND,  VWND,  XMASS, YMASS )

      IF ( LPRT ) CALL DEBUG_MSG( '### MERRA2_NESTED: a GET_TRA_MASS' )
      ...
      IF ( LPRT ) CALL DEBUG_MSG( '### MERRA2_NESTED: Before TPCORE' )
      ...
      ! The levels order is reversed only when written out in diag3.f
      ! (ccc, 3/8/10), (adj32_022)
      CALL TPCORE_MERRA2_WINDOW( D_DYN, Re, IIPAR, JJPAR,
     &                          LLPAR,    JFIRST,    JLAST,    NG,
     &                          MG,       N_TRACERS, Ap,       Bp,
     &                          UTMP,     VTMP,      P_TP1,    P_TP2,
     &                          P_TEMP,   STT(:,:,LLPAR:1:-1,:),
...
!!! (lzh, 05/08/2014)
c$$$      CALL TPCORE_MERRA2_WINDOW( D_DYN, Re, IM_W1, JM_W1,
c$$$     &                          LLPAR,    JFIRST,    JLAST,    NG,
c$$$     &                          MG,       N_TRACERS, Ap,       Bp,
c$$$     &     UTMP(I0_W1+1:I0_W1+IM_W1,J0_W1+1:J0_W1+JM_W1,:),
c$$$     &     VTMP(I0_W1+1:I0_W1+IM_W1,J0_W1+1:J0_W1+JM_W1,:),
...
c$$$     &     1                                     )
      ...
      ! Return to calling program
      END SUBROUTINE DO_MERRA2_WINDOW_TRANSPORT
...
! add merra2 nested simulation (mps, 11/13/2015)
      ...
      SUBROUTINE INIT_MERRA2_WINDOW_TRANSPORT
!
!******************************************************************************
!  Subroutine INIT_GEOS5_WINDOW_TRANSPORT initializes all module variables and
!  arrays for the GEOS-5 nested grid simulation.  This routine is only called
...
!
      ...
      USE TPCORE_BC_MOD,           ONLY : I2_W, J2_W, IM_W, JM_W
      USE TPCORE_BC_MOD,           ONLY : IGZD, INIT_TPCORE_BC
      USE TPCORE_MERRA2_WINDOW_MOD, ONLY : INIT_MERRA2_WINDOW
      USE TRACER_MOD,              ONLY : N_TRACERS
...
#     include "CMN_GCTM"  ! Re
      ...
      ! Call INIT routine from "tpcore_geos5_fvdas_window_mod.f"
      CALL INIT_MERRA2_WINDOW( IIPAR, JJPAR,    LLPAR, JFIRST,
     &                        JLAST, NG,       MG,    DBLE( N_DYN ),
     &                        Re,    YMID_R_W )

!! (lzh, 05/08/2014)
!      CALL INIT_MERRA2_WINDOW( IM_W1, JM_W1,    LLPAR, JFIRST,
!     &                        JLAST, NG,       MG,    DBLE( N_DYN ),
!     &                        Re,  YMID_R_W( J0_W1:(J0_W1+JM_W1+1) ))

      ! Return to calling program
      END SUBROUTINE INIT_MERRA2_WINDOW_TRANSPORT
...
#elif defined( GRID05x0625 )
         CALL DO_MERRA2_WINDOW_TRANSPORT_ADJ   !(mps,11/13/2015)
#elif defined( GRID025x03125 )
...
c$$$     &     TCVV,  ND24,  ND25, ND26, -1 )
      ...
      ! Reset surface pressure and ensure mass conservation
      !=================================================================
      IF ( LPRT ) CALL DEBUG_MSG( '### GFP_NESTED_ADJ: af TPCORE' )
      ...
      !### Debug
      IF ( LPRT ) CALL DEBUG_MSG( '### GFP_NESTED_ADJ: a TPCORE' )
...
! add merra2 nested simulation (mps, 11/13/2015)
      ...
      SUBROUTINE DO_MERRA2_WINDOW_TRANSPORT_ADJ
!
!******************************************************************************
!  Subroutine DO_GEOS5_WINDOW_TRANSPORT_ADJ is the driver routine for
!  the nested grid GEOS-5 transport adjoint (zhe 11/28/10)
...
c$$$     &     TCVV,  ND24,  ND25, ND26, -1 )
      ...
      ! Reset surface pressure and ensure mass conservation
      !=================================================================
      IF ( LPRT ) CALL DEBUG_MSG( '### M2_NESTED_ADJ: af TPCORE' )
      ...
      !### Debug
      IF ( LPRT ) CALL DEBUG_MSG( '### M2_NESTED_ADJ: a TPCORE' )

      ! Return to calling program
      END SUBROUTINE DO_MERRA2_WINDOW_TRANSPORT_ADJ
!------------------------------------------------------------------------------
