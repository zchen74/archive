! $Id: tagged_ox_mod.f,v 1.1 2010/03/09 15:03:47 daven Exp $
...
!$OMP+SCHEDULE( DYNAMIC )
     ...
     &                         GET_AREA_CM2(J) / DTCHEM
            ...
            !===========================================================
            ! adj_group:  apply scaling factor to source term (dkh, 03/08/10)
            IF ( LADJ ) THEN
               STT(I,J,L,N) = STT(I,J,L,N)
     &                      + PP(I,J,L,N) * EMS_SF(I,J,L,IDADJ_POx) - LL
               !! dkh debug
               !IF ( I == IFD .and. J == JFD .and. L == LFD ) THEN
               !   print*, ' PP SUM fwd = ', SUM(PP(I,J,L,:)), N
               !ENDIF
            ELSE
