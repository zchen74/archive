...
%-----------------------%
    ...
    load /home/grifbake/chen3274/testout_L13/data/landmap.mat landmap;
     landmap(landmap==2)=nan; %0 for ocean and 1 for land, 2 is for ocean%
   
    create_distmat_ocean  
...
%---------------------------------------%
	...
	flux1 = ncread(infile,'ocn_flux_opt'); 
...
flux1=temp; clear temp;    
	...
	flux1 = flux1(landmap==0); %note there will be only just one column%
...
%---------------------------%
	...
	theta0 = [std(flux),8,5700];
        theta0 =[sqrt(0.012),8,5700];
	z = flux;
